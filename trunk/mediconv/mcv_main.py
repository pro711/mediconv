...
import ocv_init
from mcv_in_progress import ProgressWindow
from ocv_transcoder import Transcoder 
from ocv_media_checker import MediaChecker
from mcv_util import confirm_overwrite, about_dialogue
import ocv_constants
from mcv_info import app_name, xmlpath, localepath, pixmappath
...
class Main:
    def __init__(self):
        ...
        self.builder.add_from_file(xmlpath)
...
#            "on_app_window_destroy" : self._on_quit,
            ...
            "on_radiobuttonfile_toggled" : self._on_radio_button_toggled
            }

        self._window = self.builder.get_object("app_window")
        self._file_chooser_button = self.builder.get_object("filechooserbutton")
        self._folder_chooser_button = self.builder.get_object("folderchooserbutton")
        self._save_folder_button = self.builder.get_object("save_folder_button")
        self._outfile_entry = self.builder.get_object("outfile_entry") 
        self._video_quality_slider = self.builder.get_object("video_quality_slider")
        self._video_quality_label = self.builder.get_object("video_quality_label")
        self._audio_quality_slider = self.builder.get_object("audio_quality_slider")
        self._audio_quality_label = self.builder.get_object("audio_quality_label")
        self._go_button = self.builder.get_object("go_button")
        self._format_combobox = self.builder.get_object("format_combobox")
        self._format_label = self.builder.get_object("format_label")
        self._container_combobox = self.builder.get_object("container_combobox")
        self._container_expander = self.builder.get_object("container_expander")
        self._radio_button_file = self.builder.get_object("radiobuttonfile")
        self._radio_button_folder = self.builder.get_object("radiobuttonfolder")
        ...
        self._window.set_title("Mediconv")
    ...
    def _on_convert_clicked(self, button):
        isfile = self._radio_button_file.get_active()
        isfolder = self._radio_button_folder.get_active()
        if isfile:
            self._on_go(button)
        elif isfolder:
    ...
    def _on_radio_button_toggled(self, button):
        ...
        if active:
            # disable folder chooser
            self._folder_chooser_button.set_sensitive(False)
            self._file_chooser_button.set_sensitive(True)
            # update file infomation
            self._on_file_changed(self._file_chooser_button)
        else:
            self._file_chooser_button.set_sensitive(False)
            self._folder_chooser_button.set_sensitive(True)
            # set sensitive
    ...
    def _set_up_filechooser(self):     
        ...
        # set up folder chooser options
        self._folder_chooser_button.set_action(gtk.FILE_CHOOSER_ACTION_SELECT_FOLDER)
        self._folder_chooser_button.set_sensitive(False)
        # set up radio buttons
        self._radio_button_folder.set_group(self._radio_button_file)
