...
def about_dialogue(window=None):
    """
    Pops up a standard GTK About dialogue. Grabs all the info from mcv_info.
    """
    dialogue = gtk.AboutDialog()
    dialogue.set_transient_for(window)
    
    dialogue.set_name("OggConvert")
    dialogue.set_authors(mcv_info.authors)
    dialogue.set_artists(mcv_info.artists)
    dialogue.set_version(mcv_info.version)
    dialogue.set_copyright(mcv_info.copyright)
    dialogue.set_website(mcv_info.website)
    dialogue.set_license(mcv_info.licence) # Learn to spell, GTK!
    ...
    logo = gtk.gdk.pixbuf_new_from_file_at_size(
            os.path.join(mcv_info.pixmappath,"oggconvert.svg"), 128, 128)
